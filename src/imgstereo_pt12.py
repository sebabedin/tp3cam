...
class CheStereoCamera(object):
    ...
    # cameraMatrix1 = self.left.camera.p
    # cameraMatrix2 = self.right.camera.p
    # distCoeffs1 = self.left.camera.d
    # distCoeffs2 = self.rigth.camera.d
    ...
    # fundMatrix = np.array([[4.567507458136527e-08, -3.930495370357416e-05, 0.010750771532659317], [4.227537878312907e-05, -8.607826196991683e-07, 0.3201405456504413], [-0.010999824926761303, -0.3182113833954986, 1]])
        ...
        # print(self.reproject_points.shape)
...
# /        self.disparity = cv::normalize(self.disparity, dst, 0, 255, NORM_MINMAX, CV_8UC1);
    ...
    def CreateReprojectPointCloud(self):
        ...
        # Return the PointCloud2 message
        # w, h, d = self.reproject_points.shape
        # points = self.reproject_points.reshape(w*h, 1, 3)

        return point_cloud2.create_cloud_xyz32(pc_msg.header, self.reproject_points)
...
class TP3Node(Node):
    ...
    def PubStereoReproject(self):
        self.pub_stereo_reproject.publish(self.stereo.CreateReprojectPointCloud())
